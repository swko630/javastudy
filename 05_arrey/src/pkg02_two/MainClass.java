...
public class MainClass {
  ...
  public static void method1() {
     ...
     *   2) int 배열이름[][]
     * 2. 생성
     *   1) new int[m][n] - 길이가 n인 1차원 배열이 m개 있는 배열 (길이가 일정한 가로형 배열 생성) 
    ...
    // 1번째 1차원 배열의 요소 3개
    System.out.println(arr[0][0]); // arr[0] 자체가 배열 이름임
 ...
 // 2번째 1차원 배열의 요소 3개
  ...
  public static void method2() {
    ...
    System.err.println(arr[0].length); // 3
    
    System.out.println(arr[1]);        // 참조값
    System.err.println(arr[1].length); // 3
  }
    ...
    for(int i = 0; i < arr.length; i++) {
      for(int j = 0; j < arr[i].length; j++) {
       // System.out.printf("%3d", arr[i][j]);
        System.out.print(String.format("%3d", arr[i][j])); // 실제 개발에선 String으로 
      } System.out.println();
    }
  }
