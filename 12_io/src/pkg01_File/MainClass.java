 ...
 * 3. 디렉터리, 파일의 정보(이름, 경로, 크기, 최종수정일 등)
 *   
 */
   ...
   */
   ...
   public static void method1() {
     ...
     // File 객체 생성 (parent는 폴더 child는 파일
...
//       dir.deleteOnExit();       // JVM 실행이 종료된 이후에 삭제한다.
   ...
   public static void method2() {
     ...
     // 파일 만들기/삭제하기 -> 폴더부터 만들고 그 안에 파일 만들기
     ...
     if(file.exists()) { // -> 파일이 있으면
      ...
      try {
       file.createNewFile(); // IOException이 checked exception라 예외처리를 해야한다. 
      } catch (Exception e) {
       e.printStackTrace();
      }
     }
   ...
   public static void method3() {
      ...
      * 5. lastModified : 최종수정일을 long 타입으로 반환(Timestamp 값)
     ...
     for(File file : files) {
       ...
       // 유형
       builder.append(String.format("%-10s", file.isDirectory() ? "파일폴더" : "파일"));
  ...
  public static void practice2(File dir) {
    ...
    // C:\2024 디렉터리 삭제하기 (비어있는 디렉터리만 삭제할 수 있다.)
